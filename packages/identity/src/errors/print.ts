...
export const safePrintType = (value: any) => {
  try {
    ...
    if (typeof value === 'object') {
      ...
      if ('length' in value) return `${constructorName}(${value.length})`;
...
export const safePrintValue = (value: any, maxLength: number = 100) => {
  try {
    ...
    if (typeof value === 'object') {
      if ('length' in value && value.length > maxLength) return '[too big to print]';
    }

    if (value !== value) // eslint-disable-line no-self-compare
      return 'NaN';
    if (typeof value === 'function') return '[source code]';
